...
- `window.context.getHTML (selector, attrs, callback)` is a method for obtaining a content of node from the parent window. The first parameter is css selector from which the content will be received. The second parameter - the list of tag attributes, which will be left in a result string (for example: class, id). The third parameter - the callback that receives the element content as a string.
